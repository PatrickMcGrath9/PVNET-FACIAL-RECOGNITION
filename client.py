...
import ttsmanager
import facemanager
from databasemanager import DatabaseManager  # <-- Added
...
class Client:  # TODO make singleton
    ...
    def __init__(self):
        self.current_frame = None
        self.database_manager = DatabaseManager()  # <-- Added
        self.face_manager = facemanager.FaceManager(self.database_manager)  # <-- Changed
        #self.tts_manager = ttsmanager.TTSManager()
        self.window = cv2.VideoCapture(0, cv2.CAP_ANY)
        self.set_window()
    ...
    def set_window(self):
        self.window.set(cv2.CAP_PROP_FRAME_WIDTH, self.params.FRAME_WIDTH)
        self.window.set(cv2.CAP_PROP_FRAME_HEIGHT, self.params.FRAME_HEIGHT)
        self.window.set(cv2.CAP_PROP_FPS, self.params.FRAME_FPS)
        self.window.set(cv2.CAP_PROP_AUTO_EXPOSURE, 3)
