...
#             if image_name.endswith(('.jpg', '.jpeg', '.png', '.JPG')):
...
# Load existing encodings if they exist, if not initialize them
if os.path.exists("encodings.pkl"):
    with open("encodings.pkl", "rb") as f:
        known_face_encodings, known_face_names, processed_images = pickle.load(f)
else:
...
# Calculate the total number of images in the database
total_images = sum(
    len([name for name in os.listdir(os.path.join(database_path, person_name)) 
        if name.endswith(('.JPG', '.jpg', '.jpeg', '.png'))])
    for person_name in os.listdir(database_path) 
    if os.path.isdir(os.path.join(database_path, person_name))
)

print(f"Total images in database: {total_images}")
encoded_count = len(processed_images)
print(f"Encoded {encoded_count} images, should encode {total_images - encoded_count} more.")
...
for person_name in os.listdir(database_path):
    ...
    if os.path.isdir(person_folder):
        for image_name in os.listdir(person_folder):
            if image_name.lower().endswith(('.jpg', '.jpeg', '.png')):  # Case-insensitive check
                print(f"Looking at {image_name}")
                image_path = os.path.join(person_folder, image_name)
                relative_image_path = os.path.relpath(image_path, database_path)
                ...
                if relative_image_path in processed_images:
                    skip_count += 1
                    print(f"Already encoded, skipping. Skip count = {skip_count}")
                    continue
                ...
                largest_face_index = max(range(len(face_locations)), key=lambda i: 
                    (face_locations[i][2] - face_locations[i][0]) * (face_locations[i][1] - face_locations[i][3])
                )

                # Encode only the largest face
                largest_face_encoding = face_encodings[largest_face_index]
                known_face_encodings.append(largest_face_encoding)
                known_face_names.append(person_name)
                processed_images.add(relative_image_path)

                encoded_count += 1
                processed_count += 1
                print(f"Encoded {encoded_count}/{total_images} images. This is the {processed_count}th encoded in this run.")
